...
- Debug route-status and route-probe smoke tests where a diagnostic request reads the live server's effective layout, then asks it to synthesize an edge crossing, deliver `MouseAbs` and continued `MouseMove` packets to the connected client, and wait for acknowledgements.
